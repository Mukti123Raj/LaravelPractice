        ...
        <div class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="{{ route('student.notifications') }}">
...
document.addEventListener('DOMContentLoaded', function() {
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            ...
                            <div>
                                ...
                                ${isUnread ? '<span class="badge bg-success ms-1">New</span>' : ''}
                            </div>
                        </div>
    ...
    function deleteNotification(notificationId) {
        if (confirm('Are you sure you want to delete this notification?')) {
            fetch(`{{ route('student.notifications.delete', ':id') }}`.replace(':id', notificationId), {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json'
    ...
    clearAllBtn.addEventListener('click', function() {
        if (confirm('Are you sure you want to delete all notifications?')) {
            fetch('{{ route("student.notifications.delete-all") }}', {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json'
